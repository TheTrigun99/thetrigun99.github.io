...
Le 3e cas est celui qui donne des conditions pour avoir un coefficient valant purement 0 en fonction de la valeur de $\lambda$ par rapport aux données.
...
$$
Ce qu'il ne faut pas oublier c'est que dans $r_j$ on a $\beta_{-j}$ et que par conséquent on va devoir fixer tous les autres paramètres pour trouver $\beta_j$. L'algorithme n'est donc plus qu'une itération sur les $\beta_k$. Ici on implémente comme critère de convergence un nombre max d'itérations (**par exemple, on passe m fois sur les p coefficients**), la convergence pouvant être plus ou moins compliqué (coordinate descent naïf n'est pas l'algorithme le plus optimisé) sans réel critère: KKT, Safe screening gap.
